...
describe 'cdap::repo' do
  context 'on Centos 6.9 x86_64 using default version' do
    ...
    it 'adds cdap-5.0 yum repository' do
      expect(chef_run).to add_yum_repository('cdap-5.0')
    end
  ...
  context 'on Ubuntu 14.04 using default version' do
    ...
    it 'adds cdap-5.0 apt repository' do
      expect(chef_run).to add_apt_repository('cdap-5.0')
    end
